...
# ===========================================================================
# 0. RUTAS BASE (compatible con PyInstaller y ejecucion con Python)
# ===========================================================================
def base_dir() -> Path:
    """
    Directorio base del programa:
    - Si corre como ejecutable PyInstaller: la carpeta donde esta el .exe.
    - Si corre con Python: la carpeta donde esta este archivo .py.
    Todos los recursos externos (.env, facturas/, salida/, BD, logs) se
    resuelven relativos a este directorio, NUNCA al cwd de turno.
    """
...
def sanitizar_nombre(nombre: str) -> str:
    """
    Hace un nombre seguro para carpetas/archivos en Windows:
    elimina <>:"/\\|?* y caracteres de control, recorta espacios y puntos.
    """
    limpio = re.sub(r'[<>:"/\\|?*\x00-\x1f]', "_", str(nombre)).strip().rstrip(".")
    return limpio or "SIN_IDENTIFICADOR"
...
class Config:
    ...
    modelo_llm: str = "gemini-3.5-flash"
    ...
    def __post_init__(self):
        # Rutas absolutas respecto al directorio base (exe o script)
        self.directorio_entrada = resolver_ruta(self.directorio_entrada)
        self.directorio_salida = resolver_ruta(self.directorio_salida)
        self.db_path = resolver_ruta(self.db_path)
        self.json_historial = resolver_ruta(self.json_historial)
        self.log_path = resolver_ruta(self.log_path)
    ...
    def validar_api_key(self):
        if not self.gemini_api_key or self.gemini_api_key == "tu_clave_de_gemini_aqui":
            raise ValueError(
                "Configura GEMINI_API_KEY en el archivo .env junto al programa "
...
def setup_logging(cfg: Config) -> logging.Logger:
    ...
    ch.setLevel(getattr(logging, cfg.log_nivel_consola.upper(), logging.INFO))
    ch.setFormatter(fmt)
    ...
    fh.setLevel(getattr(logging, cfg.log_nivel_archivo.upper(), logging.DEBUG))
    fh.setFormatter(fmt)
...
class LLMParser:
    """
    Las instrucciones al agente se envian con etiquetas estructuradas
    (estilo XML) que delimitan claramente el rol, la tarea, la entrada,
    las reglas, las validaciones y el formato de salida esperado.
    Esto reduce la dispersion del agente y lo mantiene enfocado
    exclusivamente en la extraccion de la factura.
    """

    PROMPT_TEMPLATE = """<ROLE>
Eres un auditor contable especializado en facturas de comercio exterior
(Commercial Invoices) de Baker Hughes, emitidas desde USA o Mexico.
Tu unica funcion es extraer datos con precision quirurgica.
</ROLE>

<TASK>
Analiza el texto extraido de UNA Commercial Invoice (contenido en <INPUT>)
y reconstruye su estructura en tres bloques: Encabezado, Items de producto
y Pie de pagina. El texto puede estar desordenado porque proviene de un PDF
con tablas complejas. No realices ninguna otra tarea.
</TASK>
...
<RULES>
1. DUPLICIDAD DE ITEMS:
   - Cada producto aparece DOS VECES en la factura:
     * PRIMERA aparicion: codigo + descripcion tecnica limpia + cantidad +
       precio unitario + extended price.
     * SEGUNDA aparicion: la misma descripcion (a veces ligeramente
       diferente) + metadatos.
   - Incluye el producto UNA SOLA VEZ, con la descripcion de la PRIMERA
     aparicion.
   - De la SEGUNDA aparicion extrae: Delivery, Order, Serial No.,
     commodity codes, ECCN, License Type, Country of Origin.

2. EXCLUSIONES - NO son items de producto:
   - Filas 'CARTON' o 'PIWOODENBOX' (empaques).
   - Filas que solo contengan pesos parciales (Gross Weight, Net Weight,
     Dimensions) sin codigo de producto.
   - Filas de subtotales o freight.

3. DESCRIPCION LIMPIA:
   - Solo el nombre tecnico del producto de la PRIMERA aparicion.
   - Ejemplos correctos: 'WINDOW,TEFLON (1673MA)',
     'O-RING 234 FKM 95 EL07 BLUE-DOT', 'BEARING DRIVESHAFT UPR 1673MA',
     'INSTMT 1677EA ACOUSTIC 8 CHANNEL ELECNS',
     'BELLOWS/PRESSURE COMPENSATOR, RUBBER DCBIL',
     'COVER TERMINAL MOTOR 562/MSP2-3, M42 SERIES'.
   - NO incluir en la descripcion: Delivery, Order, Export/Import commodity
     code, ECCN, License Type, Country of Origin, Serial No.,
     MEXICAN CUSTOMS IMPORT.

4. CAMPOS AUSENTES - REGLA DE VALOR POR DEFECTO:
   - Si un campo NO aparece en la factura, usa EXACTAMENTE el valor por
     defecto definido en el schema de salida:
     * numero_referencia: valor de 'Delivery: XXXXXX'. Si no existe:
       'NO TIENE DELIVERY'.
     * numero_orden: valor de 'Order: XXXXXX'. Si no existe:
       'NO TIENE ORDEN'.
     * numero_serie: valor de 'Serial No. XXXXXX'. Si no existe:
       'NO TIENE SERIAL'.
     * customs_import_mexico: valor de 'MEXICAN CUSTOMS IMPORT: XXXXXX'.
       Si no existe: 'NO APLICA'.
     * codigo_exportacion: numero puro del Export country commodity code.
       Si no existe: 'NO TIENE CODIGO DE EXPORTACION'.
     * codigo_importacion: numero puro del Import country commodity code.
       Si no existe: 'NO TIENE CODIGO DE IMPORTACION'.
     * pais_origen: pais del Country of Origin. Si no existe: 'N/A'.
     * encabezado.numero_orden: orden general de la factura. Si no existe:
       'NO DISPONIBLE'.

5. PRECIOS Y PORCENTAJES:
   - precio_unitario: valor de la columna 'Unit Price'.
   - valor_total_item (Extended Price) = Cantidad x Precio Unitario.
   - Si existe porcentaje de descuento o adicional en el item, incluirlo en
     'porcentaje'; si no hay, dejarlo en null.

6. PIE DE PAGINA:
   - Net Value: suma pura de todos los Extended Price.
   - Inland Freight / International Freight: valor si aparece; si no, 0.
...
- No hay items duplicados (mismo codigo de producto repetido).
- Ningun item es una fila de empaque, peso parcial, subtotal o freight.
- Ningun campo obligatorio quedo como string vacio: usa el valor por
  defecto del schema cuando el dato no exista en la factura.
- No inventaste ningun dato que no aparezca en <INPUT>.
</VALIDATIONS>

<OUTPUT>
Responde UNICAMENTE con un JSON que cumpla el schema FacturaInternacional
(encabezado, items, pie). Sin texto adicional, sin markdown, sin
explicaciones.
</OUTPUT>"""
    ...
    def parsear(self, texto_pdf: str) -> FacturaInternacional:
        prompt = self.PROMPT_TEMPLATE.format(texto_pdf=texto_pdf)
...
# ===========================================================================
# 10. EXPORTACIONES (CARPETA INDIVIDUAL POR FACTURA + CONSOLIDADOS)
# ===========================================================================
class ExportManager:
    """
    ...
    |   `-- reporte.xlsx      (reporte legible para revision humana)
    |-- 4638447/
    |   `-- ...
    ...
    Cada factura queda aislada en su propia carpeta; los consolidados
    solo resumen la corrida actual y no mezclan archivos individuales.
    """
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _filas_items(proc: FacturaProcesada) -> List[Dict[str, Any]]:
        f = proc.factura
        filas = []
        for item in f.items:
            filas.append({
    ...
    @staticmethod
    def _fila_encabezado(proc: FacturaProcesada) -> Dict[str, Any]:
        f = proc.factura
        return {
            "Numero_Factura": f.encabezado.numero_factura,
    ...
    # ------------------------------------------------------------------
    # Carpeta individual por factura
    # ------------------------------------------------------------------
    def exportar_factura(self, proc: FacturaProcesada):
        """Genera salida/<id_factura>/{cuerpo.csv, encabezado.csv, reporte.xlsx}."""
        numero = proc.factura.encabezado.numero_factura
        carpeta = Path(self.cfg.directorio_salida) / sanitizar_nombre(numero)
        try:
            carpeta.mkdir(parents=True, exist_ok=True)

            filas_items = self._filas_items(proc)
            if filas_items:
                pd.DataFrame(filas_items).to_csv(
                    carpeta / "cuerpo.csv", index=False, encoding="utf-8-sig"
                )
            pd.DataFrame([self._fila_encabezado(proc)]).to_csv(
                carpeta / "encabezado.csv", index=False, encoding="utf-8-sig"
            )
            self._excel_individual(proc, carpeta / "reporte.xlsx")

            self.logger.info(f"Salida individual generada: {carpeta}")
        except Exception as e:
            # Un error de escritura de UNA factura no debe detener las demas
            self.logger.error(f"Error generando archivos de la factura {numero}: {e}")

    def _excel_individual(self, proc: FacturaProcesada, ruta: Path):
        if not EXCEL_DISPONIBLE:
            self.logger.warning("openpyxl no instalado. Sin reporte.xlsx individual.")
            return
        ...
        # --- HOJA 1: INFORMACION GENERAL + VALIDACIONES ---
        ws = wb.active
        ws.title = "Informacion General"

        def seccion(titulo: str, fila: int) -> int:
        ...
        def par(clave: str, valor: Any, fila: int) -> int:
            ws.cell(row=fila, column=1, value=clave).font = Font(bold=True)
            ws.cell(row=fila, column=2, value=valor)
            return fila + 1

        fila = seccion(f"FACTURA {f.encabezado.numero_factura}", 1)
        fila = par("Fecha", f.encabezado.fecha_factura, fila)
        fila = par("Moneda", f.encabezado.moneda, fila)
        fila = par("Emisor", f.encabezado.emisor_usa, fila)
        fila = par("Comprador", f.encabezado.comprador_colombia, fila)
        fila = par("Modo de Transporte", f.encabezado.modo_transporte, fila)
        fila = par("Forwarder", f.encabezado.agente_forwarder, fila)
        fila = par("N Orden", f.encabezado.numero_orden, fila)
        fila = par("Pais Destino", f.encabezado.pais_destino_final, fila)
        fila += 1

        fila = seccion("VALORES Y TOTALES", fila)
        fila = par("Peso Bruto", f"{f.pie.peso_bruto_total_lb} / {f.pie.peso_bruto_total_kg}", fila)
        fila = par("Peso Neto", f"{f.pie.peso_neto_total_lb} / {f.pie.peso_neto_total_kg}", fila)
        fila = par("Net Value", f.pie.valor_neto, fila)
        fila = par("Freight Inland", f.pie.freight_inland, fila)
        fila = par("Freight Internacional", f.pie.freight_internacional, fila)
        fila = par("Impuestos", f.pie.impuestos, fila)
        fila = par("Total CPT", f.pie.valor_total_factura, fila)
        for r in range(fila - 7, fila):
            ws.cell(row=r, column=2).number_format = '"$"#,##0.00'
        fila += 1

        fila = seccion("COMPROBACIONES MATEMATICAS", fila)
        fila = par("Suma de items calculada", proc.suma_items_calculada, fila)
        fila = par("Net Value reportado", f.pie.valor_neto, fila)
        dif_neto = round(abs(proc.suma_items_calculada - f.pie.valor_neto), 2)
        fila = par("Diferencia (Nivel 1)", dif_neto, fila)
        total_esp = round(f.pie.valor_neto + f.pie.freight_inland
                        + f.pie.freight_internacional + f.pie.impuestos, 2)
        fila = par("Total esperado (Neto+Freight+Imp.)", total_esp, fila)
        fila = par("Total CPT reportado", f.pie.valor_total_factura, fila)
        dif_total = round(abs(total_esp - f.pie.valor_total_factura), 2)
        fila = par("Diferencia (Nivel 2)", dif_total, fila)
        celda = ws.cell(row=fila, column=1, value="Resultado")
        celda.font = Font(bold=True)
        estado = ws.cell(row=fila, column=2,
                         value="VALIDA" if proc.es_valida else "CON ERRORES")
        estado.fill = GREEN_FILL if proc.es_valida else RED_FILL
        estado.font = Font(bold=True)
        fila += 2

        fila = seccion("ADVERTENCIAS Y ERRORES", fila)
        if not proc.errores and not proc.advertencias:
            fila = par("Sin observaciones", "", fila)
        for adv in proc.advertencias:
            c = ws.cell(row=fila, column=1, value="ADVERTENCIA")
            c.fill = YELLOW_FILL
            c.font = Font(bold=True)
            ws.cell(row=fila, column=2, value=adv)
            fila += 1
        for err in proc.errores:
            c = ws.cell(row=fila, column=1, value="ERROR")
            c.fill = RED_FILL
            c.font = Font(bold=True)
            ws.cell(row=fila, column=2, value=err)
            fila += 1
        ...
        headers = [
            "Codigo", "Descripcion", "Cantidad", "Unidad", "Precio Unitario",
            "Valor Total", "Estado", "Total Calculado", "Diferencia",
            "N Ref.", "N Orden", "N Serie", "Cod. Export", "Cod. Import",
            "Pais Origen", "Customs MX"
        ]
        ws2.append(headers)
        ...
        for r_idx, item in enumerate(f.items, start=2):
            ws2.append([
                item.codigo_producto, item.descripcion_tecnica_limpia,
                item.cantidad, item.unidad_medida, item.precio_unitario,
        ...
        for col in ws2.columns:
            ...
            ws2.column_dimensions[get_column_letter(col[0].column)].width = min(max(max_len + 4, 12), 50)

        wb.save(ruta)

    # ------------------------------------------------------------------
    # Consolidados de la corrida (comportamiento historico del sistema)
    # ------------------------------------------------------------------
    def exportar(self, procesadas: List[FacturaProcesada]):
        if not procesadas:
            return
        self._csv_consolidado(procesadas)
        self._json_consolidado(procesadas)
        self._excel_consolidado(procesadas)

    def _csv_consolidado(self, procesadas: List[FacturaProcesada]):
        items_rows = []
        enc_rows = []
        for proc in procesadas:
            items_rows.extend(self._filas_items(proc))
            enc_rows.append(self._fila_encabezado(proc))
        ...
        self.logger.info("CSV consolidados exportados.")

    def _json_consolidado(self, procesadas: List[FacturaProcesada]):
        ...
        self.logger.info("JSON consolidado exportado.")

    def _excel_consolidado(self, procesadas: List[FacturaProcesada]):
        if not EXCEL_DISPONIBLE:
            self.logger.warning("openpyxl no instalado. Sin exportacion Excel consolidada.")
            return
        ...
        wb.save(out)
        self.logger.info(f"Excel consolidado exportado: {out}")
...
class ReportGenerator:
    def generar(self, procesadas: List[FacturaProcesada], fallidas: List[Tuple[str, str]],
                omitidas: List[str], logger: logging.Logger):
        total_pdf = len(procesadas) + len(fallidas) + len(omitidas)
        ...
        logger.info(f"  Omitidas (ya procesadas):      {len(omitidas)}")
        logger.info(f"  ----------------------------------------")
        ...
        if omitidas:
            logger.info("FACTURAS OMITIDAS (ya estaban en BD; usa --reprocesar para forzar):")
            for num in omitidas:
                logger.info(f"   - {num}")
...
class InvoicePipeline:
    def __init__(self, cfg: Config):
        ...
        self.parser = LLMParser(cfg, self.logger)
        self.sanitizer = DataSanitizer(self.logger)
        self.normalizer = DescriptionNormalizer(self.logger)
        self.validator = InvoiceValidator(cfg, self.logger)
        self.db = DatabaseManager(cfg, self.logger)
        self.exporter = ExportManager(cfg, self.logger)
        self.historial = HistorialManager(cfg, self.logger)
        self.reporter = ReportGenerator()

    def _listar_pdfs(self) -> List[str]:
        """Lista los PDF de la carpeta de entrada, ordenados por nombre."""
        return sorted(glob.glob(os.path.join(self.cfg.directorio_entrada, "*.pdf")))

    def _seleccionar(self, factura: Optional[str], cantidad: Optional[int]) -> List[str]:
        archivos = self._listar_pdfs()

        if factura:
            # Coincidencia por numero dentro del nombre del archivo
            filtrados = [a for a in archivos if factura in Path(a).stem]
            if not filtrados:
                self.logger.error(
                    f"No se encontro ningun PDF cuyo nombre contenga '{factura}' "
                    f"en {self.cfg.directorio_entrada}"
                )
                return []
            return filtrados
    ...
    def run(self, factura: Optional[str] = None, cantidad: Optional[int] = None,
            reprocesar: bool = False):
        archivos = self._seleccionar(factura, cantidad)

        if not archivos:
            self.logger.warning("No se encontraron archivos PDF para procesar.")
            return
        ...
        for pdf_path in archivos:
            ...
            try:
                ...
                fac = self.parser.parsear(texto)

                # Identificador alternativo si no se pudo determinar el numero
                sin_numero = not fac.encabezado.numero_factura or not fac.encabezado.numero_factura.strip()
                if sin_numero:
                    alternativo = f"SIN_NUMERO_{sanitizar_nombre(Path(pdf_path).stem)}"
                    self.logger.warning(
                        f"No se pudo determinar el numero de factura en '{pdf_path}'. "
                        f"Se usara el identificador alternativo '{alternativo}'."
                    )
                    fac.encabezado.numero_factura = alternativo

                numero = fac.encabezado.numero_factura

                # Deteccion de facturas ya procesadas
                if self.db.ya_procesada(numero) and not reprocesar:
                    self.logger.info(
                        f"Factura {numero} ya fue procesada. Se omite "
                        f"(usa --reprocesar para forzar)."
                    )
                    omitidas.append(numero)
                    continue

                if self.db.ya_procesada(numero) and reprocesar:
                    self.logger.warning(
                        f"Factura {numero} ya existe en BD. "
                        f"Se reprocesara y actualizara con los nuevos datos."
                    )

                # Saneamiento de campos ausentes
                fac = self.sanitizer.sanitizar(fac)

                fac = self.normalizer.normalizar(fac)
                proc = self.validator.validar(fac)

                if sin_numero:
                    proc.advertencias.append(
                        f"Numero de factura no detectado; se uso el identificador "
                        f"alternativo '{numero}' (archivo: {os.path.basename(pdf_path)})."
                    )

                self.db.guardar(proc)
                self.historial.guardar(proc)
                self.exporter.exportar_factura(proc)
                procesadas.append(proc)

                if proc.errores:
                    self.logger.warning(f"Factura {numero} tiene errores.")
        ...
        self.reporter.generar(procesadas, fallidas, omitidas, self.logger)
        self.logger.info("\nPara consultar una factura usa: --buscar NUMERO")


# ===========================================================================
# 14. FUNCION DE BUSQUEDA (API publica)
# ===========================================================================
def buscar_factura(numero_factura: str, cfg: Optional[Config] = None):
    cfg = cfg or Config()
...
# ===========================================================================
# 15. LINEA DE COMANDOS (argparse)
# ===========================================================================
def _entero_positivo(valor: str) -> int:
    try:
        n = int(valor)
    except ValueError:
        raise argparse.ArgumentTypeError(f"'{valor}' no es un numero entero valido.")
    if n <= 0:
        raise argparse.ArgumentTypeError("La cantidad debe ser un entero mayor que cero.")
    return n
...
def construir_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agente_comex",
        description=(
            "Sistema de automatizacion de facturas de comercio exterior. "
            "Extrae, valida y exporta la informacion de las Commercial Invoices "
            "en PDF ubicadas en la carpeta de entrada."
        ),
        epilog=(
            "Ejemplos:\n"
            ...
            "  python agente_comex.py --entrada facturas --salida salida --log-level DEBUG\n"
    ...
    grupo.add_argument(
        "--todas",
        action="store_true",
        help="Procesa todas las facturas PDF disponibles en la carpeta de entrada "
             "(comportamiento por defecto si no se indica --factura ni --cantidad)."
    )
    grupo.add_argument(
        "--factura",
        metavar="NUMERO",
        help="Procesa exclusivamente la factura indicada. El numero se busca en el "
             "nombre de los archivos PDF de la carpeta de entrada."
    )
    grupo.add_argument(
        "--cantidad",
        type=_entero_positivo,
        metavar="N",
        help="Procesa como maximo N facturas (en orden alfabetico de archivo)."
    )
    ...
    parser.add_argument(
        ...
        default="facturas",
        metavar="RUTA",
        help="Carpeta con los PDF de entrada (default: 'facturas')."
    )
    parser.add_argument(
        "--salida",
        default="salida",
        metavar="RUTA",
        help="Carpeta donde se generan los resultados (default: 'salida')."
    )
    parser.add_argument(
        "--reprocesar",
        action="store_true",
        help="Fuerza el reprocesamiento de facturas que ya existen en la base de "
             "datos, actualizando sus datos y archivos de salida."
    )
    parser.add_argument(
        "--buscar",
        metavar="NUMERO",
        help="Consulta una factura ya procesada en la base de datos y termina "
             "(no procesa PDFs ni requiere conexion con el LLM)."
    )
    parser.add_argument(
        "--log-level",
        choices=["DEBUG", "INFO", "WARNING", "ERROR"],
        default="INFO",
        help="Nivel de detalle de los logs en consola (default: INFO)."
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    cfg = Config(
        ...
        log_nivel_consola=args.log_level,
    )

    # Consulta puntual en BD: no requiere API key ni procesamiento
    if args.buscar:
        buscar_factura(args.buscar, cfg)
        return 0

    try:
        pipeline = InvoicePipeline(cfg)
